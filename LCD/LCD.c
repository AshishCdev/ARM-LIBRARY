...
char_str cus_chr [8]={
              {0x0,0xa,0x15,0x11,0xa,0x4,0x0,0x0},	//hex value for putting at the first location of CGRAM and so on 
...
/*__________________________________________this function deletes last one char on display _______________________________________________  */
void LCD_bkspc(void){
				unsigned add=Bussy_read(add_count);
				LCD_command(0x10);
				LCD_putc(' ');
